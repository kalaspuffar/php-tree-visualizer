...
# Single EXIT trap that cleans up every temp file the script makes.
# Functions register their tempfiles by pushing the path onto
# TMPFILES; the trap iterates the array on exit.
TMPFILES=()
...
# LC_ALL=C silences the perl maintainer-script locale warnings on
# hosts whose locale is set but not generated (Swedish, etc.).
# Output stays meaningful; warnings would have been ~30 lines of
# noise per apt invocation.
export LC_ALL=C
